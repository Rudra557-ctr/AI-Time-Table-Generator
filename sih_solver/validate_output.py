...
not exist as separate hard constraints in this codebase):
  ...
  HC10  room type match (equipment mismatch reported as WARNING, matching the
        solver's own known fallback for courses with no fully-compatible room)
...
def validate(timetable_csv_path, dataset_root):
    ...
    students = _read(root / "students.csv")
    student_enrollments = _read(root / "student_enrollments.csv")
    elective_groups = _read(root / "elective_groups.csv")
    elective_group_courses = _read(root / "elective_group_courses.csv")
    ...
    blocked = blocked_assignments(_read(root / "fixed_events.csv"), time_slots)
    vfs = valid_faculty_slots(_read(root / "faculty_availability.csv"))
    vrs = valid_room_slots(_read(root / "room_availability.csv"))
    ...
    for i in range(len(sessions)):
        for j in range(i + 1, len(sessions)):
            ...
            if a["section_id"] == b["section_id"] and not is_alt(a["offering_id"], b["offering_id"]):
                violations.append(f"HC03 section double-booking: {a['section_id']} has {a['offering_id']}#{a['session']} and {b['offering_id']}#{b['session']} both at {sorted(overlap)}.")
